...
function Users() {
    ...
    const getAllUsers = async () => {
        fetch(`https://expense-tracker-d2dz.onrender.com/auth/users`).then((res) => {
            return res.json();
        }).then((result) => {
            console.log(result)
            setUsers(result)
    ...
    const accountActivateDeactiate = (e, id, active) => {
        ...
        fetch("https://expense-tracker-d2dz.onrender.com/auth/activate_deactivate", { method: "PUT", headers: { "Content-type": 'Application/Json' }, body: JSON.stringify({ id: id, active: activeFlag }) }).then((res) => {
            return res.json();
        }).then((result) => {
            getAllUsers();
        })
